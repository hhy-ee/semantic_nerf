...
class My_Dataset(object):
    def __init__(self, scene_dir, args, img_h=None, img_w=None, sample_step=1, save_dir=None, mode="nyu40"):
        ...
        total_num = min(valid_data_num, 1000)
        train_step = total_num // 20
        test_step = total_num // 200
        # 10 frame
        if "complete" not in args.config_file: 
            train_ids = list(range(0, total_num, test_step))[:20]
            test_ids = [x+int(test_step//2.1) for x in list(range(0, total_num, test_step))][:200]
        # 10 frame complete
        if "complete" in args.config_file: 
            train_ids = list(range(0, 1000, train_step))[:20]
            test_ids = [x+int(test_step//2.1) for x in list(range(0, total_num, test_step))][:200]
